package org.keycloak.example.authentication;

import static org.keycloak.example.authentication.MySimpleRiskBasedAuthenticatorFactory.FAILED_LOGIN_ATTEMPTS_BEFORE_2FA;
...
public class MySimpleRiskBasedAuthenticator implements Authenticator {
    ...
    private RiskScore calculateRiskScore(KeycloakSession session, AuthenticationFlowContext context, UserModel user) {
        ...
        UserLoginFailureModel loginFailure = session.loginFailures().getUserLoginFailure(realm, user.getId());
